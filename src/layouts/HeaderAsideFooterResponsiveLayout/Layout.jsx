...
import { Button,Icon,Dialog,Balloon} from '@icedesign/base';
...
export default class HeaderAsideFooterResponsiveLayout extends Component {
  ...
  componentWillMount(){
    if (!UserService.me().isLogin()) {
      //进行页面跳转
      hashHistory.push('/login');
      return;
    }
    const nickname = UserService.me().get('nickname');
    const dept = UserService.me().get('obs').name;
    this.setState({
      nickname, dept
    })
  }
